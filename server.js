...
MYSTERY_REWARDS=[...Array.from({length:10},(_,i)=>ITEMS.find(x=>x.id===`mystery_ultra_${String(i+1).padStart(2,'0')}`)).filter(Boolean),...Array.from({length:90},(_,i)=>ITEMS.find(x=>x.id===`mystery_drop_${String(i+1).padStart(2,'0')}`)).filter(Boolean)];
...
function makeAccount(overrides={}){return {money:100000,xp:0,owned:[],profile:{country:'República Dominicana',bio:'',instagram:'',tiktok:'',youtube:'',x:'',telegram:'',discord:''},email:'',friends:[],friendRequests:[],generators:{},generatorSeconds:0,generatorSecondsDate:new Date().toISOString().slice(0,10),usernameChangedAt:null,...overrides};}
...
function renameAccount(oldName,newName){
  ...
  const a=db.accounts[oldName];
  db.accounts[newName]=a; delete db.accounts[oldName];
  for(const acc of Object.values(db.accounts)){
    acc.friends=(acc.friends||[]).map(x=>x===oldName?newName:x);
    acc.friendRequests=(acc.friendRequests||[]).map(x=>x===oldName?newName:x);
  }
  db.globalChat=(db.globalChat||[]).map(m=>m.from===oldName?{...m,from:newName}:m);
  const fc={};
  for(const [k,msgs] of Object.entries(db.friendChats||{})){
    const parts=k.split('::').map(x=>x===oldName?newName:x).sort();
    fc[parts.join('::')]=(msgs||[]).map(m=>m.from===oldName?{...m,from:newName}:m);
  }
  db.friendChats=fc;
  db.duelInvites=(db.duelInvites||[]).map(i=>({...i,from:i.from===oldName?newName:i.from,to:i.to===oldName?newName:i.to}));
  for(const [stake,n] of matchmaking.entries())if(n===oldName)matchmaking.set(stake,newName);
  for(const m of matches.values())m.players=m.players.map(x=>x===oldName?newName:x);
  for(const [token,n] of sessions.entries())if(n===oldName)sessions.set(token,newName);
}

async function api(req,res,p){
  if(p==='/api/health')return json(res,200,{ok:true,version:'1.5.0',onlinePlayers:sessions.size});
  ...
  if(p==='/api/me'&&req.method==='GET'){const name=authUser(req);if(!name)return json(res,401,{error:'Sesión no válida.'});saveDb();return json(res,200,{name,account:publicAccount(db.accounts[name])});}
  ...
  if(p==='/api/login'&&req.method==='POST'){
    const b=await body(req),name=cleanName(b.name),pass=String(b.password||''),a=db.accounts[name];if(!a)return json(res,401,{error:'Cuenta no encontrada.'});if(!a.passwordHash||hashPassword(pass,a.salt)!==a.passwordHash)return json(res,401,{error:'Contraseña incorrecta.'});saveDb();const token=crypto.randomBytes(32).toString('hex');sessions.set(token,name);return json(res,200,{token,name,account:publicAccount(a)});
  }
  ...
  if(p==='/api/settings'&&req.method==='GET'){
    ...
    return json(res,200,{email:a.email||'',username:name,usernameChangedAt:a.usernameChangedAt||null});
  }
  if(p==='/api/settings/username'&&req.method==='POST'){
    if(!name)return json(res,401,{error:'Sesión no válida.'});
    const b=await body(req),newName=cleanName(b.name);
    if(!/^[a-z0-9_]{3,20}$/.test(newName))return json(res,400,{error:'Usuario: 3-20 caracteres (a-z, 0-9, _).'});
    if(newName===name)return json(res,400,{error:'Ese ya es tu usuario actual.'});
    if(db.accounts[newName])return json(res,409,{error:'Ese usuario ya existe.'});
    const a=db.accounts[name],last=a.usernameChangedAt?Date.parse(a.usernameChangedAt):0;
    if(last && Date.now()-last<30*24*3600000){const days=Math.ceil((30*24*3600000-(Date.now()-last))/86400000);return json(res,429,{error:`Solo puedes cambiar tu nombre una vez al mes. Faltan aproximadamente ${days} días.`});}
    a.usernameChangedAt=new Date().toISOString();
    renameAccount(name,newName);saveDb();
    return json(res,200,{ok:true,name:newName,account:publicAccount(db.accounts[newName])});
  }
  if(p==='/api/settings/password-reset'&&req.method==='POST'){
    if(!name)return json(res,401,{error:'Sesión no válida.'});
    const a=db.accounts[name];
    if(!a.email)return json(res,400,{error:'Tu cuenta no tiene correo registrado.'});
    const token=crypto.randomBytes(32).toString('hex');resetTokens.set(token,{name,expires:Date.now()+30*60000});
    const base=process.env.PUBLIC_URL||`http://localhost:${PORT}`;const link=`${base}/?reset=${token}`;const sent=await sendRecoveryEmail(a.email,link);
    return json(res,200,{ok:true,message:sent?'Revisa tu correo para cambiar tu contraseña.':'La recuperación por correo necesita configurar RESEND_API_KEY y FROM_EMAIL en Render.'});
  }
  ...
  if(p==='/api/duel/invite'&&req.method==='POST'){
    if(!name)return json(res,401,{error:'Sesión no válida.'});const b=await body(req),target=cleanName(b.name),stake=Number(b.stake),a=db.accounts[name];if(!db.accounts[target])return json(res,404,{error:'Jugador no encontrado.'});if(target===name)return json(res,400,{error:'No puedes retarte a ti mismo.'});if(!isFriend(name,target))return json(res,403,{error:'Primero deben ser amigos.'});if(!DUEL_STAKES.includes(stake))return json(res,400,{error:'Apuesta no válida.'});if(Number(a.money||0)<stake)return json(res,400,{error:'No tienes suficiente dinero virtual.'});if(!hasDuelLoadout(a)||!hasDuelLoadout(db.accounts[target]))return json(res,400,{error:'Ambos jugadores necesitan al menos un artículo en cada categoría para competir.'});
  ...
  if(p==='/api/duel/respond'&&req.method==='POST'){
    if(!name)return json(res,401,{error:'Sesión no válida.'});const b=await body(req),id=String(b.id||''),accept=!!b.accept;db.duelInvites=db.duelInvites||[];const i=db.duelInvites.find(x=>x.id===id&&x.to===name);if(!i)return json(res,404,{error:'Invitación no encontrada o vencida.'});db.duelInvites=db.duelInvites.filter(x=>x.id!==id);if(!accept){saveDb();return json(res,200,{ok:true,accepted:false});}if(Number(db.accounts[name].money||0)<i.stake||Number(db.accounts[i.from].money||0)<i.stake){saveDb();return json(res,400,{error:'Uno de los jugadores ya no tiene suficiente dinero virtual.'});}if(!hasDuelLoadout(db.accounts[name])||!hasDuelLoadout(db.accounts[i.from])){saveDb();return json(res,400,{error:'Ambos jugadores necesitan una pieza en cada categoría para competir.'});}const m=createMatch(i.from,name,i.stake);saveDb();return json(res,200,{ok:true,accepted:true,match:matchView(m,name)});
  }
  // -------- MATCHMAKING 100% JUGADORES REALES --------
  if(p==='/api/match/queue'&&req.method==='POST'){
    if(!name)return json(res,401,{error:'Sesión no válida.'});const b=await body(req),stake=Number(b.stake),a=db.accounts[name];if(!DUEL_STAKES.includes(stake))return json(res,400,{error:'Apuesta no válida.'});if(Number(a.money||0)<stake)return json(res,400,{error:'No tienes suficiente dinero virtual.'});if(!hasDuelLoadout(a))return json(res,400,{error:'Tu colección debe tener al menos un artículo en cada categoría.'});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`WEALTH DUELS 1.5 online on port ${PORT}`));
